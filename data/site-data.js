...
const SITE_DATA = {
  /* --- 網站基本資料 ------------------------------------------------------
     後台「後台設定」（公開 API 的 settings）若有填寫，會覆寫這裡的
     name／school／schoolEn／grade／tagline／description（見 script.js applySiteSettings）；
     空白欄位與連不到後台時，一律使用這裡的內容。 */
  site: {
    name: "大南老邦教學網",
    school: "大南國小",
  ...
  /* --- 導覽列 ------------------------------------------------------------
     後台「後台連結管理」若有 area=nav 的資料，會以它覆寫這裡的選單（見 script.js applySiteLinks）。
     靜態檔的 content 只在連不到後台時使用。 */
  nav: [
