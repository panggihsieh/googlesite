...
## 2026-09-26 — 課表日曆：後台顯示同步狀態（ICS 空則退回 Sheet）

> 診斷：內建預設公開 ICS 目前 0 筆 VEVENT，故前台 `today.source=sheet` 屬預期行為，非按鈕／CSS 問題。

- 後台「📅 課表日曆」載入時探測各 feed 的公開 iCal：總行程、本週行程、前台來源（calendar／sheet）。
- 提供「重新檢查同步」；空 ICS 或本週無行程時以警告列說明，避免誤判為未同步。
- 需 `clasp push -f`＋部署後，後台才會顯示狀態列。
